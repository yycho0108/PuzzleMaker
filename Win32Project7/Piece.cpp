...
void Piece::rotate(bool CLOCK)
{
	XFORM XF = {};

	float angle = CLOCK ? 3.14 / 2 : -3.14 / 2;
	...
	//OffsetRgn(myRgn, jLoc, iLoc);
	OffsetRgn(myClipRgn, -(physPt.j + pWidth / 2)*wndWidth / imgWidth, -(physPt.i + pHeight / 2)*wndHeight / imgHeight);
	int regionSize = GetRegionData(myClipRgn, sizeof(RGNDATA), NULL);
	void* rawData = malloc(regionSize);
	LPRGNDATA RD = (LPRGNDATA)rawData;
	GetRegionData(myClipRgn, regionSize, RD);
	DeleteObject(myClipRgn);
	myClipRgn = ExtCreateRegion(&XF, regionSize, RD);
	free(rawData);
	OffsetRgn(myClipRgn, (physPt.j + pWidth / 2)*wndWidth / imgWidth, (physPt.i + pHeight/ 2)*wndHeight / imgHeight);
